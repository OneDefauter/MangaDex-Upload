...
// Função para encontrar o ID com base no nome da Tag
function getTagId(tagName) {
    // Percorre cada "categoria" dentro de IdTagsMangaDex
    for (const category in IdTagsMangaDex) {
      const categoryTags = IdTagsMangaDex[category];
      
      // Percorre as chaves (IDs) dentro de cada categoria
      for (const tagId in categoryTags) {
        // Se o valor bater com o nome da tag, retorna a chave
        if (categoryTags[tagId] === tagName) {
          return tagId;
        }
      }
    }
    // Caso não encontre, retorne null (ou outra forma de indicar não encontrado)
    return null;
}