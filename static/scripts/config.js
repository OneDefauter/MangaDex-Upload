...
function generateChangesMessage(changes) {
    ...
    for (const [key, value] of Object.entries(changes)) {
        const friendlyName = translations.friendlyNames[key] || key; // Nome amigável ou técnico
        const oldValue = typeof value.oldValue === "boolean" ? (value.oldValue ? translations.enabled : translations.disabled) : value.oldValue;
        const newValue = typeof value.newValue === "boolean" ? (value.newValue ? translations.enabled : translations.disabled) : value.newValue;
